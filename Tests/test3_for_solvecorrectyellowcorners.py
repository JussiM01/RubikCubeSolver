...
rotate(turn(rotate(cube, make_correct_corners(cube)[1]), x, 180),
make_correct_corners(cube)[2]) == make_correct_corners(cube)[0]

as they are supposed to be."""

def test_rotations_for_solvecorrectyellowcorners(num_iter):
    count = 0
    for n in range(num_iter):
        cube = scramble(start_cube)
        output = make_correct_corners(cube)
